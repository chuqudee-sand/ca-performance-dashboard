...
export default async function ProgramCharts() {
  ...
  const insight = (rows: any[], name: string) => {
    ...
    return [`${name}: <strong>${enrolled.toLocaleString()}</strong> enrolled`, `Graduation: <strong>${rate}%</strong>`];
  };
      ...
      {[
        ...
        <div key={name} className="space-y-8">
          ...
          <section className="grid md:grid-cols-2 gap-6">
            ...
            <InsightBox title={`${name} Trend`} bullets={[`Graduation rate over cohorts`, `Data from ${new Set(data.map(r => r.Cohort)).size} cohorts`]} />
          </section>
        </div>
      ))}
    </div>
